...
def get_labeled_data(picklename, bTrain=True):
    ...
    if os.path.isfile("%s.pickle" % picklename):
        ...
        if bTrain:
            images = open(MNIST_data_path + "MNIST/train-images-idx3-ubyte", "rb")
            labels = open(MNIST_data_path + "MNIST/train-labels-idx1-ubyte", "rb")
        else:
            images = open(MNIST_data_path + "MNIST/t10k-images-idx3-ubyte", "rb")
            labels = open(MNIST_data_path + "MNIST/t10k-labels-idx1-ubyte", "rb")
...
while counter < num_tests:
    ...
    for i in range(end_time - start_time):
        test_results[:, i] = get_recognized_number_ranking(
            assignments, testing_result_monitor[i + start_time, :]
        )
...
print(
    ...
    np.mean(sum_accurracy),
    "--> standard deviation: ",
    np.std(sum_accurracy),
)
